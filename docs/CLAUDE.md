...
Front React do JustDoIt (gerenciador de tarefas). É a raiz do repo
`justdoit-frontend` e o front ÚNICO — o app vanilla foi aposentado em 2026-07-21 e
removido em 2026-07-22, quando o app saiu de `react/` e foi promovido para a raiz.
Consome o backend em `../JustDoIt` (4 serviços Spring).
...
⚠️ PENDÊNCIA: o Pages ainda está como *deploy from a branch* (`build_type: legacy`,
raiz da `main`) — verificado via `gh api repos/:owner/:repo/pages` em 2026-07-22. Nessa
config o `deploy.yml` NÃO vale e o `index.html` da raiz (entrypoint do Vite) seria servido
cru → página em branco. Trocar em Settings → Pages → Source → **GitHub Actions** antes do
merge. Até lá, o `CNAME` da RAIZ é o que sustenta o domínio: não apagar.

GitHub Pages via `.github/workflows/deploy.yml` (push na `main`), publicando
`dist`. Duas peças que não podem sumir do build:
- `public/CNAME` — o domínio próprio (justdoit-app.duckdns.org). Por isso
...
## Estrutura (feature-based, desde 2026-07-22)

O `src/` é organizado por ASSUNTO, não por tipo de arquivo. Tudo que é de uma
feature (página, componentes, hooks, lib própria) mora na pasta dela:

```
src/
  main.jsx  App.jsx            entrypoint e rotas
  api/      client.js  endpoints.js  session.js
  components/                  UI compartilhada: Ic, Sidebar, ConfirmModal,
                               DatePicker, TimePicker
  lib/      theme  toast  utils  cores        genéricos, sem dono
  styles/                      design system global (importado no main.jsx)
  features/
    auth/        pages/{Signup,Onboarding} · components/LoginForm · hooks/useConta
    landing/     pages/Home · components/{LandingPane,Overlays,FeatureShowcase}
    tasks/       pages/{Todo,TaskDetail} · components/TaskEditor
                 hooks/{useTasks,useTaskDetail} · lib/{priority,cycle}
    categories/  components/{CategoryModal,CategorySelect} · hooks/useCategories
    calendar/    pages/Calendario · components/{WeeklyCalendar,TimeBlock,
                 EventDrawer,EventSummary} · hooks/useTimeBlocks
    dashboard/   pages/{VisaoGeral,Analise} · components/{CategoryChart,
                 DeviationChart,RateRing} · hooks/useAnalytics
    notes/       pages/Anotacoes · components/NoteComposer · hooks/useNotas
    settings/    pages/Configuracoes
```

Três decisões que não são óbvias pelo desenho:

- **`categories/` é feature própria**, não filha de `tasks/`. `useCategories` é
  consumido por tasks, calendar, dashboard, settings E pela `Sidebar` — enterrá-lo
  dentro de `tasks/` faria quatro features importarem de dentro de uma quinta.
- **`session.js` vive em `api/`, não em `features/auth/`.** Quem o importa é o
  `api/client.js` (para o refresh de token). Se ele morasse na feature, a camada
  compartilhada passaria a depender de uma feature — inversão de dependência.
- **Import cruzado entre features usa o alias `@/`** (`@/components/Ic`,
  `@/features/tasks/hooks/useTasks`), configurado em `vite.config.js` (`resolve.alias`)
  e no `jsconfig.json` (autocomplete do editor). Nada de `../../../`. Imports dentro
  da MESMA feature podem continuar relativos (`./TimeBlock`).

Cruzamentos de feature que são deliberados e devem continuar existindo:
`calendar/EventSummary` usa o `TaskEditor` de `tasks/` (é o mesmo corpo de edição —
ver seção "Editor de tarefa compartilhado"), e `tasks/Todo` usa o `NoteComposer` de
`notes/`.
...
- `features/tasks/lib/priority.js` — UI `urgent/important/normal/low` ↔ enum `URGENT_IMPORTANT/
  NOT_URGENT_IMPORTANT/NORMAL/URGENT_NOT_IMPORTANT` (bijeção; low = URGENT_NOT_IMPORTANT).
- `features/tasks/lib/cycle.js` — recorrência usa o enum do backend direto: `DAILY/WEEKLY/MONTHLY/ANNUAL`
...
A rota `/` é `features/landing/pages/Home.jsx`: **landing à esquerda, login à direita**. Substitui
o `index.html` vanilla da raiz, que ocupava a tela toda e mandava o visitante
para uma tela de login separada.

- `features/landing/components/LandingPane.jsx` — port do `index.html` + `landing/landing.js`.
...
- `src/styles/pages/home.css` tem os ajustes de meia tela. ⚠ Use **`padding-block`**
...
`features/tasks/components/TaskEditor.jsx` é o corpo ÚNICO de edição de tarefa. Quem usa:
- `features/tasks/pages/TaskDetail.jsx` — casca (Sidebar + topbar) em volta de `<TaskEditor />`.
- `features/calendar/components/EventSummary.jsx` — drawer lateral, `<TaskEditor compacto />`.
...
- **Análise e os números da Visão geral**: o backend NÃO tem analytics
  ...
  no cliente em `features/dashboard/hooks/useAnalytics.js` — **o único ponto a trocar** quando os
...
- **Tema**: a preferência "Sistema" é a AUSÊNCIA da chave `jdi.tema` (mesma
  convenção do app antigo). Ver `preferenciaTema()` / `definirTema()` em `src/lib/theme.js`.
...
- `useConta()` (`features/auth/hooks/useConta.js`) usa a queryKey `['usuario']`, a mesma da
